...
(define (assoc x n v)
  (if (eq? (caar n) x)
      (caar v) ; get current value
      (assoc x (cdr n) (cdr v))))

; ex 5.1
(define (update n v x y)
  (if (eq? (caar n) x)
      (cons (cons y (caar v)) (cdr v)) ; store new value & set current value as previous value
...
(define (prev-val e n v)
  (if (eq? (caar n) (name e))
      (cdar v) ; get previous value
      (prev-val e (cdr n) (cdr v))))

(define (val e n v)
  (cond
    ((isvar e) (assoc (name e) n v))
    ((eq? (car e) 'prev) (prev-val (cadr e) n v)) ; ex 5.1
    ((isconst e) (number e))
