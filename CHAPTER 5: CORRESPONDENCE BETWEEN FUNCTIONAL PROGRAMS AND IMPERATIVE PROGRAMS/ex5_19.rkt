  ...
  (let ((lst '()))
    (lambda (op . args)
      (cond
        ...
        ((eq? op 'peek) (car lst))
        ((eq? op 'empty?) (null? lst))))))
(define scope-stack (make-scope-stack))
(define (push-scope! s x)
  (s 'push x))
(define (pop-scope! s)
  (s 'pop))
(define (peek-scope s)
  (s 'peek))
(define (scope-empty? s)
  (s 'empty?))
(define current-address 0)
(define (generate-new-address)
  (set! current-address (+ current-address 1))
  current-address)
...
(define (effect S name-address-list address-value-list)
  (cond
    ...
    ((eq? (car S) 'PARALLEL-ASSIGN)
     (let ((variables (cadr S))
           (expressions (caddr S)))
       (foldl (lambda (var-expr address-value-list)
                (let ((var (car var-expr))
                      (expr (cdr var-expr)))
                  (update-value 
                    (get-address var name-address-list) 
                    (val expr name-address-list address-value-list) 
                    address-value-list)))
              address-value-list
              (map cons variables expressions))))
    ; ex 5.19
    ((eq? (car S) 'BLOCK)
     (let ((assignments (cadr S))
           (expression (caddr S))
           (new-name-address-list (copy-list name-address-list))  
           (new-address-value-list address-value-list))
       (push-scope! scope-stack new-name-address-list)  
        (lambda (assignment)
          (let ((var (car assignment))
                (expr (cadr assignment)))
            (set! new-name-address-list (add-variable var (generate-new-address) new-name-address-list))
            (set! new-address-value-list (update-value (get-address var new-name-address-list)
                                                       (val expr new-name-address-list new-address-value-list)
                                                       new-address-value-list))) assignments)
        (let ((result (effect expression new-name-address-list new-address-value-list)))
          (pop-scope! scope-stack)  
          result)))
    (else 'ERROR)))